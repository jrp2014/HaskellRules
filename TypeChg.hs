...
newEVar = EVar <$> newMVar

instance Unifiable Expr

instance Unifiable (Expr, FT)
...
tough3 =
  App
    (App (App (Var "foldl") (Var "cons")) (App (App (Var "cons") i1) (Var "el")))
...
tuple k n x x'
  | odd n = Pair (tuple k 1 x x') (tuple (k - 1) (n - 1) x x')
tuple k n x x'
  | even n = Pair (tuple k n2 x x') (tuple (k - n2) n2 x x')
  where
    n2 = n `div` 2
...
myt =
  [tuple 5 n (Var "succ") undefined | n <- [1 .. 5]] ++
...
mkf x y z =
  Lam "f" $ Lam "g" $ Lam "h" $ App (Var x) (App (Var y) (App (Var z) i1))

mkf2 x y z a =
  Lam "f" $
  Lam "g" $
  Lam "h" $ Lam "a" $ App (Var x) (App (Var y) (App (Var z) (App (Var a) i1)))
...
plus x = App (App (Var "plus") x)

if2 x y = App (App (App (Var "if") x) y)

i1 = Var "1"

eq x = App (App (Var "eq") x)
  ...
  Let
    ...
    (Lam
       ...
       (if2
          (eq (Var "n") i1)
          i1
          (plus (Var "n") (App (Var "fac") (plus (Var "n") i1)))))
    (App (Var "fac") i1)
...
baseenv =
  ...
  , ( "foldl"
    , Forall (MVar (-2)) $
      Forall (MVar (-1)) $ Fun (Fun a (Fun b a)) (Fun a (Fun (List b) a)))
  , ( "map"
    , Forall (MVar (-2)) $
      Forall (MVar (-1)) $ Fun (Fun a b) (Fun (List a) (List b)))
  , ( "."
    , Forall (MVar (-3)) $
      Forall (MVar (-2)) $
      Forall (MVar (-1)) $ Fun (Fun a b) (Fun (Fun c a) (Fun c b)))
