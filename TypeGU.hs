...
toCS :: Constr -> CS
toCS x
  | conFixity x == Infix = CS True (conString x)
toCS x = CS False (conString x)
...
class (Eq a, Data a, Read a) =>
      Unifiable a


instance Unifiable MVar

mkGS :: Data a => a -> GS
mkGS x
  | isJust (cast x :: Maybe Int) = GSInt (fromJust (cast x :: Maybe Int))
mkGS x
  | isJust (cast x :: Maybe Char) = GSChar (fromJust (cast x :: Maybe Char))
mkGS x
  | csName (toCS $ toConstr x) == "MVar" =
...
frGS x = read (s x)
  where
    s (GSCons x rl) =
      if csInfix x
        then case csName x of
               "(:)" -> "[" ++ mkList rl ++ "]"
               csx ->
                 "(" ++
                 s (head rl) ++
                 " " ++ [head (tail csx)] ++ " " ++ s (last rl) ++ ")"
        else csName x ++ " " ++ rs rl
...
gunify x y
  | x == y = Just []
gunify x@(GSCons _ [GSVar _]) y
  | not (gIsIn x y) = Just [(x, y)]
gunify x y@(GSCons _ [GSVar _])
  | not (gIsIn y x) = Just [(y, x)]
gunify (GSCons m1 rl1) (GSCons m2 rl2)
  | m1 == m2 = do
    um <- zipWithM gunify rl1 rl2
    foldM gasl [] um
gunify _ _ = Nothing
...
gIsIn x y
  | x == y = True
gIsIn x (GSCons _ rl) = True `elem` map (gIsIn x) rl
gIsIn _ _ = False

gHasVars :: GS -> Bool
gHasVars (GSCons _ rl) = any gHasVars rl
...
gsub1 t (x, y)
  | t == x = y
gsub1 (GSCons m rl) xy = GSCons m (map (`gsub1` xy) rl)
gsub1 t _ = t
...
remdup :: Eq a => [(a, a)] -> [(a, a)]
--remdup xl = filter (uncurry /=) (nub xl)
...
findmapair xl =
  ...
  where
    ac = [(x, z1, z2) | (x, z1) <- xl, (y, z2) <- xl, x == y, z1 /= z2]
