...
--return $ mkGS (to (f x')))
bp ::
     (Isomorphic a o, Wrapable o, Unifiable o)
  => (a -> a -> Bool)
  -> o
  -> o
  -> M o
bp f x y = do
  ...
  where
    ...
    f' [x, y] =
      let x' = from (frGS x)
       in let y' = from (frGS y)
           in if f x' y'
                then mkGS <$> newMVar
                else mzero

bf ::
     (Isomorphic a o, Wrapable o, Unifiable o) => (a -> a -> a) -> o -> o -> M o
bf f x y
        --a <- newMVar
        --let a' = wrap a
 = do
...
suppose expr typ = do
  ...
  case gunify mt' typ' of
    (Just x) -> Control.Monad.void (merge x)
    Nothing -> say "mzero" `seq` mzero
...
try :: i -> [i -> R o] -> R o
try expr = foldr (\f -> mplus (say "step" `seq` f expr)) mzero

consider :: [a] -> R a
consider = foldr (mplus . return) mzero
...
funcs :: [(GS, GFS)] -> M ()
funcs gfx = mapM_ func2 (reverse gfx)
